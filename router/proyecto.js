...
router.get('/', async function(req,res){
    ...
    try{
       const proyecto = await Proyecto.find().populate([{
        path:'cliente', select: 'nombre email estado'
       },{
        path:'etapas', select: 'nombre estado'
       },{
        path:'tipoproyecto', select: 'nombre estado'
       },{
        path:'universidad', select: 'nombre direccion telefono estado'
       }])
       res.send(proyecto)
    }catch(error){
        console.log(error);
